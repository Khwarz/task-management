...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased min-h-screen flex items-center justify-center
		bg-gray-100">
		<form action="{{ route('taches.update', $task->id) }}" method="POST" class="flex items-center justify-center flex-col">
			@csrf
			@method("PUT")
			<div class="flex flex-col">
				...
				<input type="text" name="name" id="name"  class="appearance-none
					...
					bg-white transition-all duration-300 ease-in-out" value="{{
					old('name') ?? $task->name }}" required autofocus/>
			</div>
			@error("name")
			<div class="text-left">
